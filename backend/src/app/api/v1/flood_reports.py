...
async def list_flood_reports(
    ...
    # Get reports
    reports_data = await crud_flood_reports.get_multi(db=db, offset=skip, limit=limit, **filters)

    # Get total count
    total = await crud_flood_reports.count(db=db, **filters)

    return FloodReportListResponse(total=total, items=reports_data["data"])
...
async def verify_flood_report(
    ...
    result = await db.execute(select(FloodReport).where(FloodReport.id == report_id))
