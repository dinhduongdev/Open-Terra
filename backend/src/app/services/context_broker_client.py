...
class ContextBrokerClient:
    def __init__(
        self, broker_url: str = None, mintaka_url: str = None, context_url: str = None, tenant: Optional[str] = None
    ):
        """
    ...
    async def get_entities(
        ...
        try:
            ...
            # Add Link header - handle both plain URLs and pre-formatted Link headers
            if self.context_url:
                if self.context_url.startswith("<"):
                    # Already formatted as Link header
                    headers["Link"] = self.context_url
                elif self.context_url.startswith("http"):
                    # Plain URL - format as Link header
                    headers["Link"] = (
                        f'<{self.context_url}>; rel="http://www.w3.org/ns/json-ld#context"; type="application/ld+json"'
                    )
    ...
    async def get_temporal_entities(
        ...
        try:
            ...
            params = {"type": entity_type, "timerel": timerel, "limit": limit}
